...
def rapid_conversion(
        Delta_p_plus: float, rad_osc_input: RadOscInput) -> float:
    # get core structure
    ...
    e_plus: float = eos.energy_density_from(
        eos.transitional_pressure
    )
    e_minus: float = eos.energy_density_from(
        eos.transitional_pressure*(1+1e-5)
    )
