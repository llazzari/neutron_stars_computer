...
class GPP:
    """Generalized Piecewise Polytropes from Boyle et al.,
    PRD 102 083027 (2020).
    Input eos as a string, using the keys from table III
    ...
    def sound_speed_squared_from(self, pressure: float) -> float:
        ...
        return 1/(1/(gamma - 1) + (1 + a)/(K*gamma) *
                  ((p_in_gcm3 - Lambda)/K)**(1/gamma - 1))
...
def coefficients_at_dividing_densities(eos: str) -> Coeffs:
    ...
    def pressure_at_dividing_densities() -> Array:
        rho_arr, k_arr, gamma_arr, lambda_arr = [
            np.array(cc) for cc in crust_core[:4]]
...
def Sly4_crust() -> Coeffs:
    rho_crust: Coeff = (0., 6.285e5, 1.826e8, 3.350e11, 5.317e11)
    K_crust: Coeff = (5.214e-9, 5.726e-8, 1.662e-6, -7.957e29, 1.746e-8)
    gamma_crust: Coeff = (1.611, 1.440, 1.269, -1.841, 1.382)
    lambda_crust: Coeff = (0., -1.354, -6.025e3, 1.193e9, 7.077e8)
    a_crust: Coeff = (0., -1.861e-5, -5.278e-4, 1.035e-2, 8.208e-3)
...
def connect_crust_core(crust: Coeffs, eos: str) -> CrustCoreCoeffs:
    def read_crust_core_coeff() -> CrustCoreData:
        path = Path(
            '/home/lucaslazzari/Dropbox/base_classes/tabulated_eos/GPP')
        path = path / 'TableIII_Boyle.dat'
...
def core_coefficients(crust: Coeffs, eos: str) -> Coeffs:
    ...
    def set_rho_core() -> tuple[float, ...]:
        return (rho0, 10**14.45, 10**14.58) if 'HEB' in eos \
    ...
    def set_core_lists() -> TupleListFloat:
        ...
        def set_k_core(i: int) -> float:
            return k_core[i]*(gamma_i[i]/gamma_i[i+1])\
        ...
        def set_a_core(i: int) -> float:
            return a_core[i] + gamma_i[i]*((gamma_i[i+1] - gamma_i[i])/(
